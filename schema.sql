-- =========================
-- COUNTRIES
-- =========================
CREATE TABLE countries (
    name TEXT PRIMARY KEY
);

-- =========================
-- POSITIONS
-- =========================
CREATE TABLE positions (
    position_id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    name VARCHAR NOT NULL
);

-- =========================
-- SUB_POSITIONS
-- =========================
CREATE TABLE sub_positions (
    name VARCHAR PRIMARY KEY,
    position_id INTEGER NOT NULL,
    CONSTRAINT subpositions_position_id_fkey
        FOREIGN KEY (position_id)
        REFERENCES positions(position_id)
);

-- =========================
-- CLUBS
-- =========================
CREATE TABLE clubs (
    club_id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    name VARCHAR NOT NULL,
    domestic_competition_id TEXT,
    stadium_name VARCHAR,
    stadium_seats INTEGER,
    squad_size INTEGER,
    average_age NUMERIC,
    foreigners_number INTEGER,
    national_team_players INTEGER
);

-- =========================
-- COMPETITIONS
-- =========================
CREATE TABLE competitions (
    competition_id TEXT PRIMARY KEY,
    name VARCHAR NOT NULL,
    url TEXT,
    is_major_national_league BOOLEAN DEFAULT FALSE,
    country_name VARCHAR NOT NULL,
    CONSTRAINT competitions_country_name_fkey
        FOREIGN KEY (country_name)
        REFERENCES countries(name)
);

-- =========================
-- PLAYERS
-- =========================
CREATE TABLE players (
    player_id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    name VARCHAR NOT NULL,
    date_of_birth DATE,
    sub_position TEXT,
    height_in_cm REAL,
    foot VARCHAR,
    current_club_id INTEGER,
    country_of_citizenship TEXT,
    image_url TEXT,
    market_value_in_eur BIGINT,
    CONSTRAINT players_current_club_id_fkey
        FOREIGN KEY (current_club_id)
        REFERENCES clubs(club_id),
    CONSTRAINT players_sub_position_fkey
        FOREIGN KEY (sub_position)
        REFERENCES sub_positions(name),
    CONSTRAINT players_country_of_citizenship_fkey
        FOREIGN KEY (country_of_citizenship)
        REFERENCES countries(name)
);

-- =========================
-- GAMES
-- =========================
CREATE TABLE games (
    game_id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    home_club_id INTEGER,
    away_club_id INTEGER,
    competition_id TEXT,
    home_club_goals INTEGER,
    away_club_goals INTEGER,
    date DATE,
    home_club_position INTEGER,
    away_club_position INTEGER,
    season VARCHAR,
    is_favorite BOOLEAN DEFAULT FALSE,
    CONSTRAINT games_home_club_id_fkey
        FOREIGN KEY (home_club_id)
        REFERENCES clubs(club_id),
    CONSTRAINT games_away_club_id_fkey
        FOREIGN KEY (away_club_id)
        REFERENCES clubs(club_id),
    CONSTRAINT games_competition_id_fkey
        FOREIGN KEY (competition_id)
        REFERENCES competitions(competition_id)
);

-- =========================
-- TRANSFERS
-- =========================
CREATE TABLE transfers (
    transfer_id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    player_id INTEGER,
    from_club_id INTEGER,
    to_club_id INTEGER,
    transfer_date DATE,
    transfer_season VARCHAR,
    transfer_fee BIGINT DEFAULT 0,
    market_value_in_eur BIGINT DEFAULT 0,
    CONSTRAINT transfers_duplicate_player_id_fkey
        FOREIGN KEY (player_id)
        REFERENCES players(player_id),
    CONSTRAINT transfers_from_club_id_fkey
        FOREIGN KEY (from_club_id)
        REFERENCES clubs(club_id),
    CONSTRAINT transfers_to_club_id_fkey
        FOREIGN KEY (to_club_id)
        REFERENCES clubs(club_id)
);

-- =========================
-- INDEXES (Supabase uyumlu)
-- =========================
CREATE INDEX idx_players_current_club_id
    ON players(current_club_id);

CREATE INDEX idx_players_country_of_citizenship
    ON players(country_of_citizenship);

CREATE INDEX idx_games_home_club_id
    ON games(home_club_id);

CREATE INDEX idx_games_away_club_id
    ON games(away_club_id);

CREATE INDEX idx_games_competition_id
    ON games(competition_id);

CREATE INDEX idx_transfers_player_id
    ON transfers(player_id);

CREATE INDEX idx_transfers_from_club_id
    ON transfers(from_club_id);

CREATE INDEX idx_transfers_to_club_id
    ON transfers(to_club_id);

CREATE INDEX idx_sub_positions_position_id
    ON sub_positions(position_id);

CREATE INDEX idx_competitions_country_name
    ON competitions(country_name);